...
function getLat (options) {
  var ret;
  var cache = options.data.root.__cache;

  ret = utils.randomFloat(-90, 90);
  ret = numbro(ret).format('0.000000');
...
function getLong (type, min, max, format, options) {
  var ret;
  var cache = options.data.root.__cache;

  ret = utils.randomFloat(-180, 180);
  ret = numbro(ret).format('0.000000');
...
var helpers = {
  ...
  lat: function (options) {
    // Try to use the cached values first, otherwise generate a new value
    var cache = options.data.root.__cache;
    var ret = cache.lat || getLat(options);
  ...
  long: function (options) {
    // Try to use the cached values first, otherwise generate a new value
    var cache = options.data.root.__cache;
    var ret = cache.long || getLong(options);
